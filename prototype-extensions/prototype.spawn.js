...
var primaryRoles = ['harvester', 'upgrader', 'builder', 'repairer'];
var secondaryRoles = ['wallRepairer', 'rampartRepairer'];
var specializedRoles = ['janitor', 'attacker', 'miner', 'lorry'];
var remoteRoles = ['claimer', 'remoteHarvester', 'remoteBuilder'];
...
StructureSpawn.prototype.spawnCreepsIfNecessary = function () {
  ...
  let wallTypeStructures = this.room.find(FIND_STRUCTURES, {
  ...
  _.forEach(primaryRoles, role => {
    if (Roles.lessThanMin(role, room)) primaryMinimumsMet = false;
  });
    ...
    for (let source of sources) {
      ...
      for (let container of containers) {
        ...
        if (
          !_.some(
            Game.creeps,
            c => c.memory.role === 'miner' && c.memory.sourceId == source.id && c.memory.containerId == container.id
          )
        ) {
  ...
  if (name == undefined && this.createRemoteBuilders === true) {
    if (Roles.lessThanMin('remoteBuilder', room)) {
  ...
  if (name == undefined) {
    for (let role of primaryRoles) {
      if (
        Roles.lessThanMin(role, room, this.primaryMin) ||
        (this.usePercentages === true && Roles.lessThanPerc(role, room))
      ) {
...
StructureSpawn.prototype.createCustomCreep = function (energy, role) {
  if (role === 'lorry' || role === 'janitor') return this.createLorry(energy, role);

  if (role === 'attacker') return this.createAttacker(energy);
...
// createLorry
StructureSpawn.prototype.createLorry = function (energy, role) {
  var body = [];
...
StructureSpawn.prototype.newCreep = function (role, body) {
...
// roleStatus
StructureSpawn.prototype.roleStatus = function (role) {
...
// avgCreepSize
StructureSpawn.prototype.avgCreepSize = function () {
  let totalSize = 0;
...
// energyStatus
StructureSpawn.prototype.energyStatus = function () {
